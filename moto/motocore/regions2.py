...
# Maybe have default URI be a default regex for same uri structure?
DEFAULT_URI_TEMPLATE = "{service}.{region}.{dnsSuffix}"
DEFAULT_SERVICE_DATA = {"endpoints": {}}
DEFAULT_REGION = "us-east-1"


class BaseEndpointDeconstructor(object):
    ...
    def deconstruct_endpoint(self, hostname):
        ...
        :param hostname: Name of the service to resolve an endpoint for
            (e.g., rds.us-west-2.amazonaws.com)

        :rtype: dict
        :return: Returns a dict containing the following keys:
            - service: (string, required) Resolved service name
            - retion: (string, required) Resolved region name
...
class EndpointDeconstructor(BaseEndpointDeconstructor):
    ...
    def deconstruct_endpoint(self, hostname):
        ...
        result = {}
        for partition in self._endpoint_data["partitions"]:
            if partition["dnsSuffix"] in hostname:
                for service in partition["services"]:
                    if service == "sqs":
                        for region in partition["services"][service]["endpoints"]:
                            ep = self._endpoint_for_partition(
                                partition, service, region
                            )
                            print(ep)

                    # HACK - service could be first or second item!!!
                    # This fails for SQS and probably a few other cases and needs to be more robust
                    temp = hostname.split(".")
                    if temp[0] == service or temp[1] == service:
                        result["service"] = SERVICE_ALIASES.get(service, service)
                        result["partition"] = partition["partition"]
                        for region, meta in partition["services"][service][
                            "endpoints"
                        ].items():
                            if meta.get("hostname") == hostname or region in hostname:
