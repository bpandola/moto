...
class MQResponse(BaseResponse):
    ...
    def _get_xformed_dict(
        ...
        if isinstance(value, dict):
            value = {xform_name(k): v for k, v in value.items()}
        return value
    ...
    def create_broker(self) -> ActionResult:
        ...
        encryption_options = self._get_xformed_dict(params, "encryptionOptions")
        engine_type = params.get("engineType")
        engine_version = params.get("engineVersion")
        host_instance_type = params.get("hostInstanceType")
        ldap_server_metadata = self._get_xformed_dict(params, "ldapServerMetadata")
        logs = params.get("logs", {})
        maintenance_window_start_time = self._get_xformed_dict(
